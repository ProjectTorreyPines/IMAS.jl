...
@testset "basic_dd_IO" begin
    # instantiate (empty) top-level and populate with a structure
    data = FUSE.dd();
    data.core_profiles = FUSE.core_profiles()
    @test typeof(data.core_profiles) == FUSE.core_profiles
    
    # try adding some data to core_profiles FDS
