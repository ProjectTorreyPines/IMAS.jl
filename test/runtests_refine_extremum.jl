...
# The robust refine is backend-agnostic via the _psi_* / _value_gradient / _hessian! adapters: it
# must work not only with a FastInterpolations interpolant but also with an Interpolations.jl one
# (e.g. as FRESCO builds). Analytic field below: nested ellipses ψ = ((R-R0)/a0)^2 + (Z/b0)^2.
@testset "robust refine backend-agnostic (Interpolations.jl)" begin
    @testset "analytic ellipse, IP interpolant" begin
        ...
        for (extremum_of, truth, seed) in cases
            R, Z = IMAS._robust_refine_extremum(itp, L, seed, extremum_of, (R0, 0.0))
...
@testset "_robust_refine_extremum arrives from arbitrary/bad seeds" begin
    @testset "far correct-side seed (analytic ellipse, no X-point)" begin
        ...
        # invalid extremum_of is rejected
        @test_throws ArgumentError IMAS._robust_refine_extremum(itp, L, (R0, 0.0), :bogus, (R0, 0.0))
    end
