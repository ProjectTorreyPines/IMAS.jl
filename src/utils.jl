...
"""
    Interpolations.CubicSplineInterpolation(x::AbstractVector{T} where T<:Real, y::AbstractVector{T} where T<:Real, args...; kw...)

Attempt to convert x::Vector to Range to feed to Interpolations.CubicSplineInterpolation
"""
function Interpolations.CubicSplineInterpolation(x::AbstractVector{T} where T <: Real, y::AbstractVector{T} where T<:Real, args...; kw...)
    if x[end]<x[1]
