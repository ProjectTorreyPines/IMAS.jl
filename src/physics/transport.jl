...
function blend_core_pedestal(profile::Vector{<:Real}, rho::Vector{<:Real}, ped_height::Real, ped_width::Real, rho_bound::Real, ; do_plot=false)

    ngrid = length(rho)
    profile_ped = FUSE.Hmode_profiles(profile[end], ped_height,profile[1],ngrid, 2.0, 1.4, ped_width)
