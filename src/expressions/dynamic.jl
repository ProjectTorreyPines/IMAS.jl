...
@store_expr dyexp["core_profiles.profiles_1d[:].electrons.density"] =
    (rho_tor_norm; electrons, _...) -> electrons.density_thermal .+ electrons.density_fast

@store_expr dyexp["core_profiles.profiles_1d[:].electrons.density_fast"] =
    (rho_tor_norm; _...) -> zero(rho_tor_norm)

@store_expr dyexp["core_profiles.profiles_1d[:].electrons.pressure_thermal"] =
    (rho_tor_norm; electrons, _...) -> pressure_thermal(electrons)

@store_expr dyexp["core_profiles.profiles_1d[:].electrons.pressure_fast_parallel"] =
    (rho_tor_norm; _...) -> zero(rho_tor_norm)

@store_expr dyexp["core_profiles.profiles_1d[:].electrons.pressure_fast_perpendicular"] =
    (rho_tor_norm; _...) -> zero(rho_tor_norm)

@store_expr dyexp["core_profiles.profiles_1d[:].electrons.pressure"] =
...
@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].z_ion"] =
    (; ion, _...) -> begin
...
@store_expr dyexp["core_profiles.profiles_1d[:].t_i_average"] =
    (rho_tor_norm; profiles_1d, _...) -> t_i_average(profiles_1d)

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].density"] =
...
@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].density_fast"] =
    (rho_tor_norm; ion, _...) -> ion.density .- ion.density_thermal

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].density_thermal"] =
    (rho_tor_norm; ion, _...) -> ion.density .- ion.density_fast

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].pressure_thermal"] =
    (rho_tor_norm; ion, _...) -> pressure_thermal(ion)

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].pressure_fast_parallel"] =
    (rho_tor_norm; _...) -> zero(rho_tor_norm)

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].pressure_fast_perpendicular"] =
    (rho_tor_norm; _...) -> zero(rho_tor_norm)

@store_expr dyexp["core_profiles.profiles_1d[:].ion[:].pressure"] =
    (rho_tor_norm; ion, _...) -> ion.pressure_thermal .+ ion.pressure_fast_parallel .+ ion.pressure_fast_perpendicular .* 2.0


@store_expr dyexp["core_profiles.profiles_1d[:].pressure_ion_total"] =
    (rho_tor_norm; profiles_1d, _...) -> pressure_thermal(profiles_1d.ion)

@store_expr dyexp["core_profiles.profiles_1d[:].pressure_thermal"] =
    (rho_tor_norm; profiles_1d, _...) -> pressure_thermal(profiles_1d)

@store_expr dyexp["core_profiles.profiles_1d[:].pressure_parallel"] =
...
@store_expr dyexp["core_profiles.profiles_1d[:].j_total"] =
    (rho_tor_norm; dd, profiles_1d, _...) -> profiles_1d.j_ohmic .+ profiles_1d.j_non_inductive

@store_expr dyexp["core_profiles.profiles_1d[:].j_tor"] =
...
@store_expr dyexp["core_profiles.global_quantities.current_bootstrap"] =
    (time; dd, core_profiles, _...) ->
        return [
            begin
                eqt = dd.equilibrium.time_slice[Float64(time0)]
...
@store_expr dyexp["core_profiles.global_quantities.current_ohmic"] =
    (time; dd, core_profiles, _...) ->
        return [
            begin
                eqt = dd.equilibrium.time_slice[Float64(time0)]
...
# core_transport #
#= ============ =#
@store_expr dyexp["core_transport.model[:].profiles_1d[:].time"] =
...
# equilibrium #
#= ========= =#
@store_expr dyexp["equilibrium.time_slice[:].global_quantities.energy_mhd"] =
    (; time_slice, _...) -> 3 / 2 * trapz(time_slice.profiles_1d.volume, time_slice.profiles_1d.pressure)

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.q_95"] =
    (; time_slice, _...) -> Interpolations.linear_interpolation(norm01(time_slice.profiles_1d.psi), time_slice.profiles_1d.q)(0.95)

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.q_axis"] =
    (; time_slice, _...) -> time_slice.profiles_1d.q[1]

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.q_min"] =
    (; time_slice, _...) -> minimum(time_slice.profiles_1d.q)

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.psi_axis"] =
    (; time_slice, _...) -> time_slice.profiles_1d.psi[1]

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.psi_boundary"] =
    (; time_slice, _...) -> time_slice.profiles_1d.psi[end]

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.magnetic_axis.r"] =
    (; time_slice, _...) -> time_slice.profiles_1d.geometric_axis.r[1]

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.magnetic_axis.z"] =
    (; time_slice, _...) -> time_slice.profiles_1d.geometric_axis.z[1]


@store_expr dyexp["equilibrium.time_slice[:].global_quantities.vacuum_toroidal_field.b0"] =
    (; dd, time_slice, _...) -> get_time_array(dd.pulse_schedule.tf.b_field_tor_vacuum, :reference, time_slice.time, :linear)

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.vacuum_toroidal_field.r0"] =
    (; dd, _...) -> dd.pulse_schedule.tf.r0

@store_expr dyexp["equilibrium.time_slice[:].global_quantities.magnetic_axis.b_field_tor"] =
...
@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.geometric_axis.r"] =
    (psi; time_slice, _...) -> (time_slice.profiles_1d.r_outboard .+ time_slice.profiles_1d.r_inboard) .* 0.5

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.geometric_axis.z"] =
    (psi; time_slice, _...) -> psi .* 0.0 .+ time_slice.global_quantities.magnetic_axis.z

@store_expr dyexp["equilibrium.time_slice[:].boundary.geometric_axis.r"] =
    (; time_slice, _...) -> begin
...
@store_expr dyexp["equilibrium.time_slice[:].boundary.geometric_axis.z"] =
    (; time_slice, _...) -> begin
...
@store_expr dyexp["equilibrium.time_slice[:].boundary.minor_radius"] =
    (; time_slice, _...) -> (time_slice.profiles_1d.r_outboard[end] - time_slice.profiles_1d.r_inboard[end]) * 0.5

@store_expr dyexp["equilibrium.time_slice[:].boundary.elongation"] =
    (; time_slice, _...) -> (time_slice.boundary.elongation_lower + time_slice.boundary.elongation_upper) * 0.5

@store_expr dyexp["equilibrium.time_slice[:].boundary.elongation_lower"] =
    (; time_slice, _...) -> time_slice.profiles_1d.elongation[end] # <======= IMAS limitation

@store_expr dyexp["equilibrium.time_slice[:].boundary.elongation_upper"] =
    (; time_slice, _...) -> time_slice.profiles_1d.elongation[end] # <======= IMAS limitation

@store_expr dyexp["equilibrium.time_slice[:].boundary.triangularity"] =
    (; time_slice, _...) -> (time_slice.boundary.triangularity_lower + time_slice.boundary.triangularity_upper) * 0.5

@store_expr dyexp["equilibrium.time_slice[:].boundary.triangularity_lower"] =
    (; time_slice, _...) -> time_slice.profiles_1d.triangularity_lower[end]

@store_expr dyexp["equilibrium.time_slice[:].boundary.triangularity_upper"] =
    (; time_slice, _...) -> time_slice.profiles_1d.triangularity_upper[end]

@store_expr dyexp["equilibrium.time_slice[:].boundary.squareness_lower_inner"] =
...
@store_expr dyexp["equilibrium.time_slice[:].boundary.squareness_lower_outer"] =
    (; time_slice, _...) -> time_slice.profiles_1d.squareness_lower_outer[end]

@store_expr dyexp["equilibrium.time_slice[:].boundary.squareness_upper_outer"] =
    (; time_slice, _...) -> time_slice.profiles_1d.squareness_upper_outer[end]

@store_expr dyexp["equilibrium.time_slice[:].boundary.squareness"] =
    (; time_slice, _...) -> begin
        mxh = MXH(time_slice.boundary.outline.r, time_slice.boundary.outline.z, 2)
        return -mxh.s[2]
    end

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.j_tor"] =
    (psi; profiles_1d, _...) -> begin
        j_parallel = profiles_1d.j_parallel
        Jpar_2_Jtor(profiles_1d.rho_tor_norm, j_parallel, true, time_slice)
    end

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.j_parallel"] =
...
@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.dpressure_dpsi"] =
    (psi; time_slice, profiles_1d, _...) -> gradient(psi, profiles_1d.pressure)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.dvolume_drho_tor"] =
    (psi; profiles_1d, _...) -> gradient(profiles_1d.rho_tor, profiles_1d.volume)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.dvolume_dpsi"] =
    (psi; profiles_1d, _...) -> gradient(psi, profiles_1d.volume)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.darea_drho_tor"] =
    (psi; profiles_1d, _...) -> gradient(profiles_1d.rho_tor, profiles_1d.area)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.darea_dpsi"] =
    (psi; profiles_1d, _...) -> gradient(psi, profiles_1d.area)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.dpsi_drho_tor"] =
    (psi; profiles_1d, _...) -> gradient(profiles_1d.rho_tor, profiles_1d.psi)

@store_expr dyexp["equilibrium.time_slice[:].profiles_1d.psi_norm"] =
    (psi; _...) -> norm01(psi)

# 2D
@store_expr dyexp["equilibrium.time_slice[:].profiles_2d[:].r"] =
    (dim1, dim2; _...) -> ones(length(dim2))' .* dim1

@store_expr dyexp["equilibrium.time_slice[:].profiles_2d[:].z"] =
...
@store_expr dyexp["equilibrium.time_slice[:].profiles_2d[:].b_field_r"] =
...
@store_expr dyexp["equilibrium.time_slice[:].profiles_2d[:].j_tor"] =
    (dim1, dim2; profiles_2d, _...) -> begin
...
@store_expr dyexp["equilibrium.time_slice[:].time"] =
...
#  core_sources  #
#= ============ =#
@store_expr dyexp["core_sources.source[:].profiles_1d[:].electrons.power_inside"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
        energy = profiles_1d.electrons.energy
        cumtrapz(profiles_1d.grid.volume, energy)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].electrons.energy"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].total_ion_power_inside"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
        total_ion_energy = profiles_1d.total_ion_energy
        cumtrapz(profiles_1d.grid.volume, total_ion_energy)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].total_ion_energy"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].electrons.particles_inside"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
        particles = profiles_1d.electrons.particles
        cumtrapz(profiles_1d.grid.volume, particles)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].electrons.particles"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].current_parallel_inside"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
        j_parallel = profiles_1d.j_parallel
        cumtrapz(profiles_1d.grid.area, j_parallel)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].j_parallel"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].torque_tor_inside"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
        momentum_tor = profiles_1d.momentum_tor
        cumtrapz(profiles_1d.grid.volume, momentum_tor)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].momentum_tor"] =
    (rho_tor_norm; profiles_1d, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].ion[:].particles_inside"] =
    (rho_tor_norm; profiles_1d, ion, _...) -> begin
        particles = ion.particles
        cumtrapz(profiles_1d.grid.volume, particles)
    end

@store_expr dyexp["core_sources.source[:].profiles_1d[:].ion[:].particles"] =
    (rho_tor_norm; profiles_1d, ion, _...) -> begin
...
@store_expr dyexp["core_sources.source[:].profiles_1d[:].time"] =
...
#  build  #
#= ===== =#
@store_expr dyexp["build.layer[:].identifier"] =
    (; build, layer, _...) -> begin
...
@store_expr dyexp["build.layer[:].outline.r"] =
    (x; layer, _...) -> opposite_side_layer(layer).outline.r

@store_expr dyexp["build.layer[:].outline.z"] =
    (x; layer, _...) -> opposite_side_layer(layer).outline.z

@store_expr dyexp["build.layer[:].shape"] =
    (; layer, _...) -> opposite_side_layer(layer).shape

@store_expr dyexp["build.layer[:].shape_parameters"] =
    (; layer, _...) -> opposite_side_layer(layer).shape_parameters

@store_expr dyexp["build.layer[:].start_radius"] =
    (; build, layer_index, _...) -> build_radii(build.layer)[1:end-1][layer_index]

@store_expr dyexp["build.layer[:].end_radius"] =
    (; build, layer_index, _...) -> build_radii(build.layer)[2:end][layer_index]

@store_expr dyexp["build.layer[:].area"] =
    (; layer, _...) -> area(layer)

@store_expr dyexp["build.layer[:].volume"] =
    (; layer, _...) -> volume(layer)

@store_expr dyexp["build.tf.ripple"] =
...
@store_expr dyexp["costing.cost_operations.system[:].yearly_cost"] =
...
@store_expr dyexp["costing.cost_operations.yearly_cost"] =
...
@store_expr dyexp["costing.cost_decommissioning.system[:].cost"] =
    (; system, _...) -> isempty(system.subsystem) ? error("no subsystem") : sum(sub.cost for sub in system.subsystem if !ismissing(sub, :cost))

@store_expr dyexp["costing.cost_decommissioning.cost"] =
...
@store_expr dyexp["pulse_schedule.tf.b_field_tor_vacuum_r.reference"] =
    (time; tf, _...) -> tf.r0 .* tf.b_field_tor_vacuum.reference

@store_expr dyexp["pulse_schedule.tf.r0"] =
    (; dd, _...) -> dd.equilibrium.vacuum_toroidal_field.r0

@store_expr dyexp["pulse_schedule.tf.b_field_tor_vacuum.reference"] =
    (time; dd, _...) -> dd.equilibrium.vacuum_toroidal_field.b0

@store_expr dyexp["pulse_schedule.tf.time"] =
    (time; dd, _...) -> dd.equilibrium.time

#= ========= =#
#  stability  #
#= ========= =#
@store_expr dyexp["stability.model[:].cleared"] =
    (time; model, _...) -> Int.(model.fraction .<= 1.0)

@store_expr dyexp["stability.all_cleared"] =
...
#  summary  #
#= ======= =#
@store_expr dyexp["summary.fusion.power.value"] = # NOTE: This is the fusion power that is coupled to the plasma
...
@store_expr dyexp["summary.global_quantities.ip.value"] =
...
@store_expr dyexp["summary.global_quantities.current_non_inductive.value"] =
    (time; dd, summary, _...) -> begin
        tmp = eltype(summary)[]
        for time in summary.time
            cp1d = dd.core_profiles.profiles_1d[Float64(time)]
...
@store_expr dyexp["summary.global_quantities.current_ohm.value"] =
    (time; dd, summary, _...) -> begin
        tmp = eltype(summary)[]
        for time in summary.time
            cp1d = dd.core_profiles.profiles_1d[Float64(time)]
...
@store_expr dyexp["summary.heating_current_drive.power_launched_ic.value"] =
...
@store_expr dyexp["summary.heating_current_drive.power_launched_lh.value"] =
...
@store_expr dyexp["summary.heating_current_drive.power_launched_nbi.value"] =
...
@store_expr dyexp["summary.heating_current_drive.power_launched_total.value"] =
    (time; dd, summary, _...) ->
...
@store_expr dyexp["summary.volume_average.n_e.value"] =
    (time; dd, summary, _...) -> begin
        tmp = eltype(summary)[]
        for time in summary.time
            cp1d = dd.core_profiles.profiles_1d[Float64(time)]
...
@store_expr dyexp["summary.volume_average.t_i_average.value"] =
    (time; dd, summary, _...) -> begin
        tmp = eltype(summary)[]
        for time in summary.time
            cp1d = dd.core_profiles.profiles_1d[Float64(time)]
...
@store_expr dyexp["summary.volume_average.zeff.value"] =
    (time; dd, summary, _...) -> begin
        tmp = eltype(summary)[]
        for time in summary.time
            cp1d = dd.core_profiles.profiles_1d[Float64(time)]
