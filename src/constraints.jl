...
function update_ConstraintFunctionsLibrary!()
    ...
    ConstraintFunction(:zero_ohmic, "MA", dd -> abs(sum(trapz(dd.core_profiles.profiles_1d[].grid.area, dd.core_profiles.profiles_1d[].j_ohmic))) / 1E6, ==, 0.0, 0.1) # absolute tolerance
    ...
    ConstraintFunction(:min_lh_power_threshold, "%", dd -> (L_H_threshold(dd) / dd.requirements.lh_power_threshold_fraction), >, 1.0)
    ...
    ConstraintFunction(:max_tf_j, "%", dd -> (1. + dd.requirements.coil_j_margin) * dd.build.tf.max_j / dd.build.tf.critical_j, <, 1.0)
    ConstraintFunction(:max_oh_j, "%", dd -> (1. + dd.requirements.coil_j_margin) * dd.build.oh.max_j / dd.build.oh.critical_j, <, 1.0)
