...
"""
    Base.resize!(@nospecialize(ids::IDSvector{T}), identifier_name::Symbol, conditions::Pair{String}...; wipe::Bool=true, allow_only_one::Bool=true)::T where {T<:IDSvectorElement}
...
NOTE: `allow_only_one` will delete all extra entries matching the conditions.

Returns the selected IDS
"""
function Base.resize!(@nospecialize(ids::IDSvector{T}), identifier_name::Symbol, conditions::Pair{String}...; wipe::Bool=true, allow_only_one::Bool=true)::T where {T<:IDSvectorElement}
    i = get(name_2_index(ids), identifier_name, nothing)
    if i === nothing
        error("`$(repr(identifier_name))` is not a known identifier for dd.$(fs2u(eltype(ids)))")
    elseif :identifier in fieldnames(eltype(ids))
        return resize!(ids, "identifier.index" => i, conditions...; wipe, allow_only_one)
    else
        return resize!(ids, "index" => i, conditions...; wipe, allow_only_one)
    end
end
