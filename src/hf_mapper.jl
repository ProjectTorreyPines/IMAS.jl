...
if case == :double
    ...
    if merge_wall
        ...
        psi_last_diverted, null_is_inside = IMAS.find_psi_last_diverted(eqt, wall_r, wall_z, PSI_interpolant) # psi of grazing surface
        ...
        add_indexes = add_indexes[(psi_wall .< psi_separatrix .|| psi_wall .> psi_wall_midplane) .||
                                (psi_wall .>= psi_separatrix .&& psi_wall .<= psi_last_diverted[2] .&& 
                                sign(eqt.boundary.x_point[end].z).*wall_z.>abs(eqt.boundary.x_point[end].z)).&& null_is_inside]
